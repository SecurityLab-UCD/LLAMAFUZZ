...
def main():
    ...
    generate_prompt = f"### Input: ```Generate a new hex {args.fuzzing_target} seed. Make sure the example is complete and valid. Only return the solution.``` "
    seed_queue=[example[args.fuzzing_target],generate_prompt,generate_prompt,generate_prompt]
    ...
    while True:
        ...
        if current_seed.startswith("### Input"):
            prompt = current_seed
            if seeds_from_fuzzer:
                current_seed = seeds_from_fuzzer.pop()
                if len(seed_queue)>30:
                    seed_queue = [generate_prompt,generate_prompt,generate_prompt]
                seed_queue.append(current_seed)
        else:
            if seeds_from_fuzzer:
                current_seed = seeds_from_fuzzer.pop()
                if len(seed_queue)>30:
                    seed_queue = [generate_prompt,generate_prompt,generate_prompt,generate_prompt,generate_prompt]
                seed_queue.append(current_seed)
                is_from_fuzzer = True
