...
if __name__ == "__main__":
    ...
    msg_npy_half = np.arange(BUFF_SIZE // 2, dtype=np.uint8).reshape(
        (2, BUFF_SIZE // 4)
    )
    try:
        ...
        #
        bytearray1 = struct.pack("I", msg_int)
        s = "THIS AABCDEFGHIJKLMOPQRSTUVW"
        mq.send(bytearray1 + s.encode("utf-8"), True, type=TYPE_INT_STRING)
        ...
        # bytearray1 = struct.pack("d", msg_double1)
        # mq.send(
        #     bytearray1 + msg_npy_half.tobytes(order="C"), True, type=TYPE_DOUBLEANDNUMPY
        # )
