...
new_model = "llama-2-7b-structured-libjpg-hex-mutator"
dataset_path = "./prompts/fine-tune-libjpeg-mutate-llm.csv"

device = Accelerator().local_process_index

def formatting_prompts_func(examples):
    print('example',examples)
    formated_prompt = f"### Input: ```Based on below libjpeg seed, mutate a new libjpeg seed. Make sure the example is complete and valid. Only return the solution, no other words. {examples[0]}```\n ### Output: {examples[1]}"
...
def create_datset(tokenizer, data_file_path):
    ...
    # dataset = dataset.map(lambda examples: tokenizer(examples["context"]), batched=True)
